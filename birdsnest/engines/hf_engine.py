########################################################################################################
# Bird's Nest — Generic HuggingFace Engine
# Handles xLSTM, StripedHyena, Granite, and any AutoModelForCausalLM-compatible model
########################################################################################################
...
class HFEngine(InferenceEngine):
    """Generic inference engine for any HuggingFace AutoModelForCausalLM model."""
    ...
    def load(self, model_path: str, **kwargs) -> Dict[str, Any]:
        """Load any HF Transformers-compatible model."""
        ...
        # If not a local directory, try resolving from catalog
        if not os.path.isdir(model_path):
            from birdsnest.models import MODEL_CATALOG
            for entry in MODEL_CATALOG:
                if entry["id"] in model_path or model_path in entry.get("name", ""):
