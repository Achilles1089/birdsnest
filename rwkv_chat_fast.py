########################################################################################################
# RWKV-6 Interactive Chat — Mac Metal GPU Accelerated Edition
# Optimized from rwkv_chat.py with:
#   - MPS (Metal) GPU acceleration
#   - float16 weights (halves memory bandwidth)
#   - Removed torch.jit in favor of plain nn.Module
########################################################################################################
...
if torch.backends.mps.is_available():
    ...
    DTYPE = torch.float32  # MPS works best with float32 for RWKV ops
...
def sample_logits(out, temperature=1.0, top_p=0.8):
    # Keep sampling on CPU (tiny tensor, numpy is fine)
...
# ── RWKV-6 RNN Model (GPU-Accelerated) ─────────────────────────────────────
...
class RWKV_RNN(nn.Module):
    def __init__(self, args):
        ...
        print(f"   Loading weights from {args.MODEL_NAME}.pth ...")
        w = torch.load(args.MODEL_NAME + '.pth', map_location='cpu')

        for k in w.keys():
            w[k] = w[k].to(dtype=self.dtype)
        ...
        for k in w.keys():
            ...
            for p in parts:
                if p.isdigit():
                    ...
                    here = getattr(here, p)
            setattr(here, last, w[k].to(self.device))

        # Count params
        total_params = sum(w[k].numel() for k in w.keys())
        mem_mb = sum(w[k].numel() * w[k].element_size() for k in w.keys()) / 1024 / 1024
        print(f"   {total_params/1e9:.2f}B params, ~{mem_mb:.0f} MB on {self.device}")
    ...
    def time_mixing(self, x, state, i:int, x_maa, w_maa, k_maa, v_maa, r_maa, g_maa, tm_w1, tm_w2, td_w1, td_w2, time_first, time_decay, kw, vw, rw, gw, ow, ln_w, ln_b):
        ...
        x = torch.zeros(H, S, device=self.device, dtype=self.dtype)
        a = k @ v
        x = r @ (time_first * a + s)
        s = a + w * s
...
╚═══════════════════════════════════════════╝
    ...
    print(f"🧠 Loading RWKV-6 1.6B model...")
    t0 = time.time()
    ...
    while True:
        ...
        print(f"\n  [{generated} tokens, {tok_s:.1f} tok/s, {DEVICE}]\n")
